...
<mapper namespace="${mapperInterface.packageName}.${baseInfo.fileName}">
    ...
    <insert id="insert" keyColumn="id" keyProperty="id" parameterType="${tableClass.fullClassName}"
            useGeneratedKeys="true">
        insert into ${tableClass.tableName}
        ( <#list tableClass.allFields as field>${field.columnName}<#if field_index%3==2>${"\n        "}</#if><#sep>,</#list>
        )
        values
        (<#list tableClass.allFields as field>${'#'}{${field.fieldName},jdbcType=${field.jdbcType}}<#if field_index%3==2>${"\n        "}</#if><#sep>,</#list>
        )
    </insert>
    <insert id="insertSelective" keyColumn="id" keyProperty="id" parameterType="${tableClass.fullClassName}"
            useGeneratedKeys="true">
        insert into ${tableClass.tableName}
